...
model = dict(
    type='FViT',
    backbone=dict(
        type='EvaCLIPViT',  # 使用新的SigLIPViT类
        model_name='EVA02-CLIP-L-14-336',
        pretrained='checkpoints/FineCLIP_coco_vitl14.pt',
        norm_cfg=norm_cfg,
    ...
    roi_head=dict(
        ...
        vlm_roi_extractor=dict(
            ...
            out_channels=768,  # SigLIP-L 使用 1024 维 (EVA-CLIP 是 768)
            featmap_strides=[14])),
    train_cfg=dict(
        rpn=dict(
            assigner=dict(
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=
        '/hy-tmp/liuzihan/datasets/Ovd4Incremental/0/train_task_1.json',
