...
class Aggregator(nn.Module):
    ...
    def forward(self, img_feats, text_feats, appearance_guidance):
        ...
        #     fg_indices = avg.topk(self.pad_len, dim=-1, sorted=False)[1] # shape = [B, pad_len]
        #     th_text = F.normalize(text_feats, dim=-1) # shape = [B, T, P, C]
        #     th_text = torch.gather(th_text, dim=1, index=fg_indices[..., None, None].expand(-1, -1, th_text.size(-2), th_text.size(-1))) # shape = [B, pad_len, P, C]
